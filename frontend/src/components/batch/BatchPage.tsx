...
function FlowVisualization() {
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 mb-6">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-4">
        ...
        <div className="flex flex-col items-center gap-1 rotate-90 md:rotate-0">
          <div className="flex items-center gap-1">
            <div className="w-10 md:w-16 h-0.5 bg-emerald-500" />
        ...
        <div className="flex flex-col items-center gap-2 w-full md:w-52">
      ...
      <div className="mt-6 bg-slate-50 rounded-xl p-4 text-center flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-0">
        <span className="text-xl sm:text-2xl font-bold text-slate-900">5 payments</span>
        <span className="text-slate-400 mx-3">&rarr;</span>
        <span className="text-xl sm:text-2xl font-bold text-emerald-600">1 transaction</span>
        <span className="text-slate-400 mx-3">&rarr;</span>
        <span className="text-xl sm:text-2xl font-bold text-teal-600">1 fee</span>
      </div>
    </div>
  );
}
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-5">
          ...
          <div className="text-2xl sm:text-3xl font-bold text-red-600">{formatCurrency(totalTraditionalCost)}</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-5">
          ...
          <div className="text-2xl sm:text-3xl font-bold text-emerald-600">{formatCurrency(totalTempoCost)}</div>
          <div className="text-xs text-slate-400 mt-1">$0.001 per batch &times; {BATCH_QUEUE_SEED.length} batches</div>
        </div>
        <div className="bg-emerald-50 rounded-xl border-2 border-emerald-300 shadow-sm p-4 sm:p-5">
          <div className="text-xs text-emerald-700 uppercase tracking-wider font-bold mb-2">Total Saved</div>
          <div className="text-2xl sm:text-3xl font-bold text-emerald-700">{formatCurrency(totalSaved)}</div>
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="text-xs text-slate-500 uppercase tracking-wider">
                ...
                <th className="text-left px-4 sm:px-6 py-3 font-medium">Payments</th>
                <th className="text-right px-4 sm:px-6 py-3 font-medium">Total</th>
                <th className="text-left px-4 sm:px-6 py-3 font-medium hidden sm:table-cell">Created</th>
                <th className="text-left px-4 sm:px-6 py-3 font-medium hidden sm:table-cell">Tx Hash</th>
                <th className="text-right px-4 sm:px-6 py-3 font-medium">Savings</th>
              </tr>
            </thead>
            <tbody>
              {BATCH_QUEUE_SEED.map((batch) => (
                ...
                >
                  <td className="px-4 sm:px-6 py-3.5 text-sm font-medium text-emerald-600">{batch.id}</td>
                  <td className="px-4 sm:px-6 py-3.5"><StatusBadge status={batch.status} /></td>
                  <td className="px-4 sm:px-6 py-3.5 text-sm text-slate-600">{batch.payments}</td>
                  <td className="px-4 sm:px-6 py-3.5 text-sm font-semibold text-slate-900 text-right">{formatCurrency(batch.total)}</td>
                  <td className="px-4 sm:px-6 py-3.5 text-sm text-slate-600 hidden sm:table-cell">{batch.created}</td>
                  ...
                  <td className="px-4 sm:px-6 py-3.5 text-sm font-bold text-emerald-600 text-right">
      ...
      {/* Expanded detail */}
      {expandedBatch && (
        <div className="mt-4 bg-white rounded-xl border border-slate-200 shadow-sm p-4 sm:p-6 animate-fade-in-up">
            ...
            {["Queued", "Constructing", "Broadcasting", "Confirming", "Settled"].map((label, i) => {
              ...
              return (
                <div key={label} className="flex items-center gap-1">
                  {i > 0 && <ArrowRight className="w-3 h-3 mr-1 hidden sm:block" />}
          ...
          <div className="grid gap-2">
            {EMPLOYEES_SEED.slice(0, BATCH_QUEUE_SEED.find((b) => b.id === expandedBatch)?.payments || 0).map((emp) => (
              <div key={emp.id} className="flex items-center justify-between bg-slate-50 rounded-xl px-3 sm:px-4 py-3">
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  <span className="text-xs text-slate-400 bg-white px-2 py-0.5 rounded hidden sm:inline">{emp.currency}</span>
